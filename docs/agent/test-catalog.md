...
| `frontend/scripts/candidate-dossier.test.mjs` | Exact-ID candidate service and route wiring, separate receipt/disbursement channels, cycle-bounded source-ledger coverage states, and candidate-directory dossier links | Populated candidate rows and rendered retry behavior |
| `frontend/scripts/component-size-budget.test.mjs` | Every app/component TSX file stays below the 1,000-line hard limit and produces the CI component-size report | Runtime performance and visual composition quality |
| `frontend/scripts/election-map-refactor.test.mjs` | Election map controller/panel separation retains the established map interaction contract | Rendered map geometry, pointer behavior, and responsive layout |
| `frontend/scripts/member-dossier-refactor.test.mjs` | Member route remains a thin composition boundary, dossier modules stay split, and sections load independently with truthful state copy | Live endpoint latency and rendered tab transitions |
| `frontend/scripts/member-dossier-utils.test.mjs` | Member tab normalization, legacy `donations`/`voting` aliases, keyboard wrapping, shareable URLs, safe download filenames, and CSV escaping | Browser history timing and actual file downloads |

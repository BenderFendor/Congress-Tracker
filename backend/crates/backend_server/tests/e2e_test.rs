...
async fn test_backend_e2e() {
    ...
    match res {
        Ok(response) => {
            ...
            assert!(status.is_success(), "Trades request failed with status {}. Body: {}", status, body);
            
            let json: Value = serde_json::from_str(&body).expect("Failed to parse trades JSON");
            assert!(json.get("data").is_some(), "Trades response missing 'data' field");
            assert!(json["data"].as_array().is_some(), "'data' field is not an array");
    ...
    let res_pol = client.get(&pol_url)
        .send()
        .await;
    ...
    match res_pol {
        Ok(response) => {
            let status = response.status();
            let body = response.text().await.unwrap();
            assert!(status.is_success(), "Politicians request failed with status {}. Body: {}", status, body);
            
            let json: Value = serde_json::from_str(&body).expect("Failed to parse politicians JSON");
            assert!(json.get("data").is_some(), "Politicians response missing 'data' field");
            assert!(json["data"].as_array().is_some(), "'data' field is not an array");
            println!("Politicians found: {}", json["data"].as_array().unwrap().len());
        }
        Err(e) => {
            let _ = server.kill();

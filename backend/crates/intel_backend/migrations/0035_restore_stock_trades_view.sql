-- Restore the canonical stock-trades materialized view if a live schema drifted
-- after migration 0015. The view is derived entirely from normalized disclosure
-- evidence, so rebuilding it does not discard source records.
...
SELECT
  ...
  CASE
    WHEN dd.filing_date >= dt.transaction_date
      THEN (dd.filing_date - dt.transaction_date)::int
    ELSE NULL
  END AS disclosure_lag_days,
  CASE
    WHEN dd.filing_date >= dt.transaction_date
      AND (dd.filing_date - dt.transaction_date) > 45
      THEN true
    ELSE false
  END AS late_filing,
...
CREATE UNIQUE INDEX stock_trades_trade_id_idx ON stock_trades (trade_id);
CREATE INDEX stock_trades_tx_date_idx ON stock_trades (transaction_date DESC NULLS LAST);
CREATE INDEX stock_trades_ticker_idx ON stock_trades (ticker) WHERE ticker IS NOT NULL;
